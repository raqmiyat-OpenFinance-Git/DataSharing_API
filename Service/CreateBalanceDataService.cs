...
namespace DataSharing_API.Service
{
    public class CreateBalanceDataService: ICreateBalanceDataService
    {
        ...
        public CreateBalanceDataService(IDbConnection idbConnection, IOptions<StoredProcedureParams> storedProcedureParams, NLogManagerService logger, BalanceDbContext postbalanceDbContext)
        {
        ...
        {
            _logger.LogInfo("GetCheckerTppConsents started.");
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error while fetching GetCheckerTppConsents");
                return new List<TppBalancesDetailDto>();
            }
            _logger.LogInfo("GetCheckerTppConsents fetched successfully.");
        ...
        {
            _logger.LogInfo("GetTppConsentsByDate started.");
            List<TppBalancesDetailDto> tppBalancesDetailDtos = new List<TppBalancesDetailDto>();
            try
            {
                var parameters = new DynamicParameters();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error while fetching GetTppConsentsByDate");
                return new List<TppBalancesDetailDto>();
            }
            _logger.LogInfo("GetTppConsentsByDate fetched successfully.");
            return tppBalancesDetailDtos;
        }

        public async Task<TppBalancesDetailDto> GetTppBalancesByIdAsync(long balanceRequestId)
        {
            _logger.LogInfo("GetTppConsentById started.");
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error while fetching GetTppConsentById");
                return new TppBalancesDetailDto();
            }
            _logger.LogInfo("GetTppConsentById fetched successfully.");
        ...
        async Task<long> ICreateBalanceDataService.SaveBalanceRequestAsync(TppBalancesRequest tppBalancesRequest)
        {
            ...
            try
            {
                ...
                _logger.LogInfo($"SaveConsentRequestAsync is done. Id:{tppBalancesRequest.BalanceRequestId})");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"CorrelationId: {tppBalancesRequest.CorrelationId} || An error occurred while saving SaveConsentRequestAsync.");
            }
        ...
        public async Task<string> SaveBalanceResponseAsync(long id, TppBalancesResponse tppBalancesResponse)
        {
            ...
            try
            {
                ...
                _logger.LogInfo($"SaveConsentResponseAsync is done. ConsentRequestId: {id}, ConsentResponseId:{tppBalancesResponse.BalanceResponseId})");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"An error occurred while saving SaveConsentResponseAsync.");
            }
            return result;
        }
        ...
        public bool ValidateConsentId(string consentId)
        {
            ...
            {
                _logger.LogError(ex, "Error while checking consent status");
                return false;
            }
        }
    }
